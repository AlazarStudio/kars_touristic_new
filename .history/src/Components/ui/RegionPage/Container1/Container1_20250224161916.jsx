...
import { debounce } from 'lodash'; // ✅ Используем lodash для debounce
...
function Container1({
  ...
  // ✅ Используем lodash debounce (300 мс задержка)
  ...
  // Следим за изменениями searchQuery и применяем debounce
  useEffect(() => {
  ...
  const filteredData = selectedData.filter((item) => {
    const matchesSearch =
      ...
      (item.places &&
        item.places.some((place) =>
          place.toLowerCase().includes(debouncedSearch.toLowerCase())
        ));
    ...
    const matchesStars =
      starFilter === '' || (item.numStars && item.numStars.toString() === starFilter);
