...
function Container1({
  ...
  const selectedData =
    selected === 0
      ? multiDayTours.filter((tour) => tour.regionId.includes(currentRegion.id))
      : selected === 1
      ? onerDayTours.filter((tour) => tour.regionId.includes(currentRegion.id))
      : selected === 2
      ? [] // Если для авторских туров появятся данные, добавьте фильтр здесь
      : selected === 3
      ? hotels.filter((hotel) => hotel.regionId.includes(currentRegion.id))
      : selected === 4
      ? places.filter((place) => place.regionId.includes(currentRegion.id))
      : selected === 5
      ? events.filter((event) => event.regionId.includes(currentRegion.id))
      : [];
