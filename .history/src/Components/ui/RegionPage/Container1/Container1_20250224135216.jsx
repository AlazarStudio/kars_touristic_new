...
function Container1({
  ...
  selected === 0
    ...
    : selected === 2
    ? [] // Можно добавить данные для авторских туров
    : selected === 3
    ? hotels.filter((hotel) => hotel.regionId === currentRegion.id) // regionId — число
    : selected === 4
    ? places.filter((place) => place.regionId === currentRegion.id) // regionId — число
    : selected === 5
    ? events.filter((event) => event.regionId === currentRegion.id) // regionId — число
    : [];


    console.log('Current Region ID:', currentRegion.id);
    console.log('Filtered Places Data:', places.filter((place) => place.regionId === currentRegion.id));
