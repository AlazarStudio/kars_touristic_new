...
function Container1({
  ...
  const selectedData =
    selected === 0
      ...
      : selected === 2
      ? [] // Добавьте фильтр, если понадобится
      : selected === 3
      ? hotels.filter((hotel) =>
          Array.isArray(hotel.regionId)
            ? hotel.regionId.includes(currentRegion.id)
            : false
        )
      : selected === 4
      ? places.filter((place) =>
          Array.isArray(place.regionId)
            ? place.regionId.includes(currentRegion.id)
            : false
        )
      : selected === 5
      ? events.filter((event) =>
          Array.isArray(event.regionId)
            ? event.regionId.includes(currentRegion.id)
            : false
        )
      : [];
