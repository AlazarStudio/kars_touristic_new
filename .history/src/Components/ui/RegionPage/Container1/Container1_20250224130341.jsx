            ...
            <div className={classes.cards}>
              {selectedData.length > 0 ? (
                selected === 0 || selected === 1 ? (
                  ...
                  selectedData.map((item) => (
                    <CardHotels item={item} swiperRef={swiperRef} key={item.id} />
                  ))
                ) : selected === 4 ? (
                  selectedData.map((item) => (
                    <CardPlaces item={item} swiperRef={swiperRef} key={item.id} />
                  ))
                ) : selected === 5 ? (
                  selectedData.map((item) => (
                    <CardEvents item={item} swiperRef={swiperRef} key={item.id} />
                  ))
                ) : (
                  // Если понадобится для категории "Авторские туры" или других
                  selectedData.map((item) => (
                    <Card1 item={item} swiperRef={swiperRef} key={item.id} />
                  ))
                )
