...
function OneTourPage() {
  ...
  const tour = [...multiDayTours, ...onerDayTours].find(
    (item) => transliterate(item.title).toLowerCase() === decodedTitle.toLowerCase()
  );
            ...
            <WidthBlock>
              <div className={classes.tourContent}>
                <div className={classes.link}>
                  ...
                  {currentRegion && (
                    <Link to={`/region/${transliterate(currentRegion.link)}`}>
                      {currentRegion.title}
                    </Link>
                  )}
                </div>
                    ...
                    <div className={classes.cardRight}>
                      ...
                      >
                        {tour.img.map((image, idx) => (
                          <SwiperSlide key={idx}>
                            <img src={image} alt={`${tour.title} - ${idx + 1}`} className={classes.cardImg} />
                          </SwiperSlide>
                        ))}
                      </Swiper>
                    </div>
