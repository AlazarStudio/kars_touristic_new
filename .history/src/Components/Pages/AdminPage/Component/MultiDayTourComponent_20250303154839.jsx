...
const ITEM_TYPE_DAY = 'DAY';

/** 🔥 Draggable Tour (Перетаскиваемый тур в списке) */
const DraggableTour = ({ tour, index, moveTour }) => {
  ...
  return (
    <tr ref={ref} style={{ opacity: isDragging ? 0.5 : 1, cursor: 'grab' }}>
      ...
      <td>
        <EditButton record={tour} />
        <DeleteButton record={tour} />
      </td>
    </tr>
  );
};

/** 🔥 Список туров с drag-and-drop */
    ...
    <DndProvider backend={HTML5Backend}>
      <List>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {tours.map((tour, index) => (
              <DraggableTour key={tour.id} index={index} tour={tour} moveTour={moveTour} />
            ))}
          </tbody>
        </table>
