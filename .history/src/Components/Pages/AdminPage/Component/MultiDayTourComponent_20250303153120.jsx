...
const DraggableDay = ({ index, moveDay, record }) => {
  ...
  return (
    <div ref={ref} style={{ opacity: isDragging ? 0.5 : 1, padding: '8px', border: '1px solid #ddd', marginBottom: '4px', cursor: 'grab' }}>
      <TextInput source={`infoByDays[${index}].title`} label="Название дня" fullWidth />
      <TextInput source={`infoByDays[${index}].description`} label="Описание" multiline fullWidth />
    </div>
  );
};
...
export const MultiDayToursEdit = () => {
  ...
  useEffect(() => {
    dataProvider.getOne('multidaytours', { id: 1 }).then(({ data }) => {
    ...
    <Edit mutationMode="pessimistic" transform={saveOrder}>
      <SimpleForm>
        ...
        <ArrayInput source="infoByDays" label="Дни тура">
          <SimpleFormIterator>
            {days.map((day, index) => (
              <DraggableDay key={day.id} index={index} moveDay={moveDay} record={day} />
            ))}
          </SimpleFormIterator>
        </ArrayInput>
